...
enum ComponentDepth { CD_ONE = 1, CD_THREE = 3};
enum BitDepth { BD_EIGHT = 8, BD_SIXTEEN = 16, BD_THIRTYTWO = 32 };

struct EXPORT_MPCDI DataMap {
  // Description:
  // public constructor/destructor
  DataMap(unsigned int sizeX, unsigned int sizeY, 
          ComponentDepth componentDepth);
  ~DataMap();
  
  // Description:
  // get functions for size
  mpcdiGetConstMacro(SizeX,unsigned int);
  mpcdiGetConstMacro(SizeY,unsigned int);
